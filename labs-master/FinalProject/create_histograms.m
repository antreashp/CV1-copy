...
%     bag=bagOfWords();
    ...
    all_counts=double.empty(0,k);
    
    for class=1:4
            for image =1:size(train_data(:,class),1)
                image*class
                [histogram,counts]=get_hist(dir_path,train_data(image,class),k,C,colorspaces,type,stepOrSIFTsamples);
%                 bag=addDocument(bag,counts)

                all_counts=cat(1,all_counts,counts);
...
%     for  i=1:size(all_counts,1)
%     
%         for j =1:size(all_counts,2)
%             if all_counts(i,j)>0
%                 all_counts(i,j)=all_counts(i,j)/max(all_counts(i,:));
% 
%             else
%                 all_counts(i,j)=all_counts(i,j)/min(all_counts(i,:));
% 
% 
%             end
%         end
...
elseif typeof=="test"
    ...
    test_data_y=double.empty(0,1);
     ...
     test_counts=double.empty(0,k);
    for class=1:4
        for image =1:size(train_data(:,class),1)
            image * class
            [histogram,counts]=get_hist(dir_path,train_data(image,class),k,C,colorspaces,type,stepOrSIFTsamples);
             test_counts=cat(1,test_counts,counts);
            test_data_x=cat(1,test_data_x,histogram);
